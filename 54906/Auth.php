...
class Auth extends CI_Controller
{
	...
	function custom_permissions()
	{
			...
			
		{
			...
			if ($this->dx_auth->get_permission_value('visitPccwSolution') != NULL AND $this->dx_auth->get_permission_value('visitPccwSolution'))
			{
				echo 'PCCW Solution Tab is allowed';
			}
			else
			{
				echo 'PCCW Solution Tab is not allowed';
			}

			echo '<br/>';

			if ($this->dx_auth->get_permission_value('visitHaPortalApp') != NULL AND $this->dx_auth->get_permission_value('visitHaPortalApp'))
			{
				echo 'HA Portal App Tab is allowed';
			}
			else
			{
				echo 'HA Portal App Tab is not allowed';
			}

			echo '<br/>';

			if ($this->dx_auth->get_permission_value('visitDev') != NULL AND $this->dx_auth->get_permission_value('visitDev'))
			{
				echo 'Dev Tab is allowed';
			}
			else
			{
				echo 'Dev Tab is not allowed';
			}

			echo '<br/>';

			if ($this->dx_auth->get_permission_value('visitRecordControl') != NULL AND $this->dx_auth->get_permission_value('visitRecordControl'))
			{
				echo 'Record Control Tab is allowed';
			}
			else
			{
				echo 'Record Control Tab is not allowed';
			}

			echo '<br/>';

			if ($this->dx_auth->get_permission_value('visitDigitalAsset') != NULL AND $this->dx_auth->get_permission_value('visitDigitalAsset'))
			{
				echo 'Digital Asset Tab is allowed';
			}
			else
			{
				echo 'Digital Asset Tab is not allowed';
			}

			echo '<br/>';

			if ($this->dx_auth->get_permission_value('visitAdministrator') != NULL AND $this->dx_auth->get_permission_value('visitAdministrator'))
			{
				echo 'Administrator Tab is allowed';
			}
			else
			{
				echo 'Administrator Tab is not allowed';
			}
	...
	// Example how to get permissions you set permission in /backend/uri_permissions/
	function uri_permissions()
	{
			...
			
		{
			...
			$hello = $this->dx_auth->get_permission_value('uri');


			if ($hello != NULL AND $hello)
			{
				// if ($hello ='/')
				// {
				// 	echo  "All is allowed";
				...
				// else
				// {
				// 	// echo  "".$hello[0]." is allowed";
					print_r(array_values($hello));
				// }

...
class Item extends CI_Controller {
	...
	public function __construct(){
		...
		if ($this->dx_auth->is_logged_in()){
	        ...
	        if(!in_array("/", $uriArray)){
	            ...
	            while($counterForUriArray < count($uriArray) AND $validation == 0) {
	                $testingUri = explode("/",$uriArray[$counterForUriArray]);

	                $counterForComparison =1;
	                while($counterForComparison<= $testingUri AND $validation == 0){
	                    if($testingUri[$counterForComparison] == $currentUri[$counterForComparison-1]){
	                        if(count($testingUri) - $counterForComparison == 0){
	                           $validation = 1;
	                        }
	                    } 
	                    else{
	...
	public function index(){
		try{
			...
			$this->mWorkflow->_callback_check_draft($this->dx_auth->get_user_id());
			$this->mWorkflow->_callback_check_otherVersions($this->dx_auth->get_user_id());
			...
			$crud->add_fields('uuid','type','name','alias','title','content','keyword','description','site','parent_uuid','template_id','version','status','menu','sequence','image','effective_date','create_date','update_date', 'author_id');
			...
			// <----- update
			$crud->edit_fields('uuid','parent_uuid','type','name','alias','title','content','keyword','description','template_id','menu','sequence','image','effective_date','update_date','status', 'author_id');
			...
			$crud->add_action('Preview', $this->config->item('base_url').'assets/images/Agreement-15.png',  $this->config->item('base_url_public').'pccwsolutions2016/public/preview/', '');
			...
			$crud->set_rules('name', 'Unique Filename','callback_check_name');
	...
	public function _callback_name($value, $row){
		if($row->type==$this->type['3']){
			return '<a href="?parent_uuid='.$row->uuid.'" class="folder">'.$row->name.'</a>';
		}
		return $row->name;
	}
	...
	public function _callback_update_date($value, $row){
		...
		$date = new DateTime('@'.$row->update_date, new DateTimeZone('UTC')); 
		if(isset($row->update_date) || $row->update_date == null || $row->update_date>= 0){
	...
	public function _get_workflow_action_button($primary_key, $row){
		...
		if($action == 'Approved'){
			...
			if ($this->dx_auth->get_permission_value('approveAndReject') != NULL AND $this->dx_auth->get_permission_value('approveAndReject')){
				$result = "<a title='Waiting for approval' style='color: #E84423;' href=".$this->config->item('base_url')."index.php/workflow/approve/itemList/".$row->uuid.">Approve</a>";
				$result .= str_repeat('&nbsp;', 1);
				$result .= "<a title='Waiting for reject' style='color: #0066cc;' href=".$this->config->item('base_url')."index.php/workflow/reject/itemList/".$row->uuid.">Reject</a>";
			}
			...
			$result = "<a id='Waiting_for_discard' style='color: #E84423;' title='Waiting for discard' href=".$this->config->item('base_url')."index.php/workflow/discard/".$row->uuid.">Discard</a>";
		}
		return $result;
	}
	...
	public function check_name(){
		$purifier						= new HTMLPurifier();			
		$check_array['name']			= $purifier->purify($_POST['name']);
		$check_array['parent_uuid']		= $_POST['parent_uuid'];
		$check_array['uuid']			= $_POST['uuid'];
		$check_array['type']			= $_POST['type'];
		$valid 							= $this->mWorkflow->_callback_check_name($check_array);

		if(!$valid){
			$this->form_validation->set_message('check_name',"Name should be unique");
		}
		return $valid;
	}
